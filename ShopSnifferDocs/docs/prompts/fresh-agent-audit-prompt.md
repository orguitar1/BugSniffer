...
Run this audit at the start of every development session, before the three agents begin work.

Paste this prompt into a completely new agent session with no prior ShopSniffer context. Do NOT share any summaries, conversation history, or prior session context with this agent.

---

You are auditing the ShopSniffer codebase as a fresh reviewer with no prior context about this project.
...
5. **Hidden Bug Risks** — Are there race conditions, state management issues, type mismatches, or assumptions baked into the code that could fail silently? Pay special attention to: scanner result handling (must use discriminated types, not bare lists), exception handling (no silent swallowing), database commit boundaries, and external API call resilience (timeouts, retries).
